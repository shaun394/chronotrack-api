...
namespace ChronoTrack.Application.Tasks.Queries.ListByProject
{
    public sealed class ListProjectTasksByProjectQueryHandler
        : IRequestHandler<ListProjectTasksByProjectQuery, IReadOnlyCollection<ProjectTaskReadModel>>
    {
        private readonly IProjectTaskReadRepository _projectTaskReadRepository;

        public ListProjectTasksByProjectQueryHandler(IProjectTaskReadRepository projectTaskReadRepository)
        {
        ...
        {
            return await _projectTaskReadRepository.ListByProjectAsync(query.ProjectId, ct);
        }
    }
}